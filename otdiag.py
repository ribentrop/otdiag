...
####################
# pathname overhead
# Работа с переменным окружения SPLUNK
'''
def get_splunk_etc():
...
# Сбор специфической для Splunk конфигурации (обработка длинных путей при сборе Lookups на платформе Windows)
'''
...
# Много много кода для работы со Splunk Rest
'''
##################
# REST!
...
# Сбор информации о процессах (Win- операционные сиситемы)
'''
...
# Информация об установленных программах Windows
'''
def get_msinfo():
...
# Собирает информацию о KVStore Splunk
'''
def kvStoreListing():
...
# Добавление  diag каталога SHclustering ( могу ошибаться)
'''
def copy_raft_dir():
...
# Копирование чепоинтов windows inputs
'''
...
# Обработка ситуаций с нестандартным местом хранения логов (Splunk)
'''
...
# Сбор специфической для Splunk конфигурации (собирать или нет etc/)
'''
...
# Сбор специфической для Splunk конфигурации (manifest files)
'''
def copy_manifests():
...
# Сбор специфической для Splunk конфигурации (var/run/splunk/cachemanger_upload.json)
'''
...
###################
# ================================================================
# === Функции и классы про выбор нужных логов исходя из опций ====
# === командной строки. И про фильтрацию внутри логов  ===========
# === "чувствительной информация". Много Splunk-специфики ========
# ================================================================
# Data filtering
...
# Похоже на фильтрацию чувствительной информации из логов
# Пока не актуально, но может быть интересно в будущем.
'''
enabled_filters = []
...
# Определяем исходя из опций, какие логи собираем, а какие нет
# Пока не актуально, но может быть интересно в будущем.
'''
...
# Какая-то хитрая непонятная мне функция фильтрации
'''
...
# Обработка паролей в passwd файле (для Splunk).
'''
...
# Фильтрация путей (исключение сбора информации по заданным путям-исключениям)
'''
...
# Сбор специфической для Splunk конфигурации (сбор только опрпделенных lookup файлов)
'''
...
# Обработка фильтра, отвечающего за размер собираемого файла (например, lookup должен быть не больше чем 10М)
'''
...
# Нормальзует вывод команды btool
'''
...
# обрабатывает имена индексов с знаком "_"
'''
...
# Формирует JSON структуру из имен индексов и путей к ним
'''
...
# Рекурсивно собирает список файлов в каталоге и подкаталогах
'''
...
# Не рекурсивно собирает список файлов в каталоге
'''
...
# Что-то еще про листинг каталога...
'''
...
# Функция для рекурсивного обхода каталога и подкаталогов и добавления результат обхода в диаг
...
# Преобразовние относительных (с переменной окружения) путей к индексам в абсолютные
'''
...
# Удаление каталогов (в случае, если они должны быть исключены из диага?)
'''
def deleteTree(path):
...
# Обработка Windows crush файлов
'''
...
#  Все, что касается сбора диагностики в приложениях (Splunk apps)
'''
app_info_dict = {}
...
#  Преобразование и парсинг величин и наименований (kb.Mg,Gb)
'''
...
def local_getopt(file_options, cmd_argv=sys.argv):
    ...
    # Описываем нужные группы опций и сами опции.
    # Ненужные пока опции комментируем
    '''
    ...
    # Класс, использующийся для сбора diag информации из Splunk - приложений.
    ...
    # Пробегаем по приложениям и собираем специфичные компоненты для каждого приложения.
    # Для нас пока не актуально, поэтому все закомментировано
    ...
    # ... оставляя обработку только нужных нам. Остальное комментируем.
...
# Обработка server.conf приложений Splunk, где есть diag stanza, в которой указано, что можно собирать в диаг, а что нет
'''
...
# обрабатывает ситуацию, когда компонент enabled, а относительный путь (по которому находится компонент) ислючен
'''
...
# Получение списка исключенных файлов
'''
def get_excluded_filelist():
    return excluded_filelist
    '''

# Оставить от исключенного файла запись о том, что файл был исключен. Включить запись в диаг
'''
...
# Добавить список исключенных файлок к диаг файлу
'''
...
# Создает tar из папки. Судя по комментариям Splunk, это deprecated code.
'''
...
# Юниты (ну или задачи), которые осуществляет функция сбора диана.
...
def create_diag(options):
    ...
    # Подготовка фильтра для исключения файлов. Разобраться, как работает.
    # Пока не используем.
    '''
    reset_excluded_filelist()
    ...
    # Опция stdout пока не используется.
    '''
    ...
    # Убеждаемся, что ОС поддерживается.
    # Пока не используем .
    '''
    ...
    # Похоже на настройки фильтров - какие логи собираем, какие нет
    # И как фильтруем чувствительную информацию внутри логов
    # Пока не актуально, но может быть интересно в будущем.
    '''
    ...
    # SPlunk REST не используем, кандидат на удаление
    '''
    ...
    try:
        ...
        # Собираем информацию о SPLUNK_HOME & SPLUNK_ETC и пишем в system_info
        ...
        # Собираем информацию о версии SPLUNK
        '''
        ...
        # Сбор информации о операционной системе (версия, ядро)
        # Полезно, но надо почистить от Windows и Splunk
        '''
        ...
        # Сбор специфической для Splunk конфигурации
        '''
        ...
        # Сбор специфической для Splunk конфигурации (KVStore)
        '''
    ...
    # Сбор специфической для Splunk конфигурации (собирать или нет логи)
    '''
    if not 'log' in options.components:
        logger.info("Skipping Splunk log files...")
    else:
    ...
    # Сбор специфической для Splunk конфигурации (собирать или нет индексы)
    '''
    ...
    # Сбор специфической для Splunk конфигурации (COMPOSITE_XML - что это?)
    ...
    # Сбор специфической для Splunk конфигурации (собирать или нет  dispatch)
    '''
    ...
    # Сбор специфической для Splunk конфигурации (consensus - что это?)
    '''
    ...
    # Специфика для Windows input checkpoint files.
    # again.. so small...
    '''
    if os.name == "nt":
    ...
    # Добавление специфичной для Splunk apps информации
    '''
    gather_app_extensions(options)
    '''

    # Формирование списка из имен исключенных файлов и включение сформированного списка в диаг
    '''
    ...
    # Формирование списка из всех ексеп-текстов, которые возникале при сборе диаг из Splunk-apps
    # и добавление сформированного списка к диагу.
...
# Все про логирование. Переделать со Splunk на  Otplatform
'''
...
# Сбор информации о Splunk apps. Пока для нас не актуально, но в далеом будущем возможно
'''
...
def main():
    ...
    # Обработка опций для удаленного сбора диага и для diag upload.
    '''
    if options.uri:
...
# Все для загрузки diag на сайт Splunk
#######
# upload logic
'''
